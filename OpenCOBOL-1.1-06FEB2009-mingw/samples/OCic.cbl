      ...
      **           TEMP variable.                                    **
      **        As a result of this change, OCic no longer runs a    **
...
GC1010     SELECT Cobc-Output          ASSIGN TO 'OC-Messages.TXT'
                                       ORGANIZATION IS LINE SEQUENTIAL.
...
GC0410        10 S-SOURCE              PIC X(1)  VALUE 'N'.
      ...
      *>
      *>===================================================================================
      *> OCic (2010/04/02 11:36) - OpenCOBOL V1.1 Interactive Compilation                01
      *> +-----------------------------------------------------------------------------+ 02
      ...
      *> +---------------------------------------------------------+-------------------+ 16
      *>   Extra "cobc" Switches, if any ("-save-temps=xxx" disables listing/xref):      17
      *> +-----------------------------------------------------------------------------+ 18
...
GC1010       05 LINE 17 COL 04 VALUE 'Extra "cobc" Switches, if any ("-s
GC1010-                              'ave-temps=xxx" disables listing/xr
GC1010-                              'ef):'.
GC1010       05 LINE 21 COL 04 VALUE 'Program Execution Arguments, if an
GC1010-                              'y:'.
      *>
...
GC1010     IF S-SOURCE NOT = ' '
GC1010     OR S-XREF   NOT = ' '
GC1010         STRING '-save-temps '
GC1010             DELIMITED SIZE INTO Cobc-Cmd
GC1010             WITH POINTER I
GC1010         END-STRING
GC1010     END-IF
...
GC0609         SET 88-Complete TO TRUE
           ELSE
GC1010         SET 88-Compile-Failed TO TRUE
GC1010         SET 88-Complete TO TRUE
GC0909         DISPLAY
GC0909             Blank-Screen
GC0909         END-DISPLAY
...
GC0909         END-DISPLAY
           END-IF
GC1010     CALL 'CBL_DELETE_FILE'
GC1010         USING 'OC-Messages.TXT'
GC1010     END-CALL
DEBUG D    DISPLAY
DEBUG D        'Deletion of OC-Messages.TXT: ' RETURN-CODE
DEBUG D        UPON SYSERR
DEBUG D    END-DISPLAY
           .
...
GC0410             END-CALL
GC0410         END-IF
GC1010         IF OS-Windows
GC1010             MOVE CONCATENATE("del ",TRIM(Prog-Name,Trailing))
GC1010               TO Cmd
GC1010         ELSE
GC1010             MOVE CONCATENATE("rm ",TRIM(Prog-Name,Trailing))
GC1010               TO Cmd
GC1010         END-IF
GC1010         CALL 'SYSTEM'
GC1010             USING CONCATENATE(TRIM(Cmd,Trailing),'.i')
GC1010         END-CALL
GC1010         CALL 'SYSTEM'
GC1010             USING CONCATENATE(TRIM(Cmd,Trailing),'.o')
GC1010         END-CALL
GC1010         CALL 'SYSTEM'
GC1010             USING CONCATENATE(TRIM(Cmd,Trailing),'.c')
GC1010         END-CALL
GC1010         CALL 'SYSTEM'
GC1010             USING CONCATENATE(TRIM(Cmd,Trailing),'.c.h')
GC1010         END-CALL
GC1010         CALL 'SYSTEM'
GC1010             USING CONCATENATE(TRIM(Cmd,Trailing),'.c.l*.h')
GC1010         END-CALL
GC0410     ELSE
...
GC1010 01  Main-Module-Name            PIC X(256).
...
GC1010        88 In-IDENTIFICATION-DIVISION VALUE "i", "I", "?".
GC1010        88 In-ENVIRONMENT-DIVISION    VALUE "e", "E".
GC1010        88 In-DATA-DIVISION           VALUE "d", "D".
GC1010        88 In-PROCEDURE-DIVISION      VALUE "p", "P".
...
DEBUG D    DISPLAY
DEBUG D        'Src-Filename    ='
DEBUG D        TRIM(Src-Filename,Trailing)
DEBUG D        UPON SYSERR
DEBUG D    END-DISPLAY
GC1010     PERFORM VARYING I FROM LENGTH(Src-Filename)
GC1010                  BY -1
GC1010                  UNTIL Src-Filename(I:1) = '/' OR '\'
GC1010                     OR I = 0
GC1010     END-PERFORM
GC1010     IF I = 0
GC1010         MOVE UPPER-CASE(Src-Filename) TO Main-Module-Name
GC1010     ELSE
GC1010         ADD 1 TO I
GC1010         MOVE UPPER-CASE(Src-Filename(I:)) TO Main-Module-Name
GC1010     END-IF
DEBUG D    DISPLAY
DEBUG D        'Main-Module-Name='
DEBUG D        TRIM(Main-Module-Name,Trailing)
DEBUG D        UPON SYSERR
DEBUG D    END-DISPLAY
...
GC1010     END-STRING
GC1010     CALL 'CBL_CHECK_FILE_EXIST'
GC1010         USING Expanded-Src-Filename
GC1010               PIC-X256
GC1010     END-CALL
GC1010     IF RETURN-CODE NOT = 0
DEBUG D        DISPLAY
DEBUG D            'File Not Found: '
DEBUG D            TRIM(Expanded-Src-Filename,Trailing)
DEBUG D            UPON SYSERR
DEBUG D        END-DISPLAY
DEBUG D        DISPLAY
DEBUG D            'Above PROBABLY caused by "-I -save-xrefs=xxx"'
DEBUG D            UPON SYSERR
DEBUG D        END-DISPLAY
GC1010         GOBACK
GC1010     END-IF
...
GC1010         IF Token-Is-Key-Word
GC1010         OR Token-Is-Reserved-Word
GC1010             MOVE SPI-Current-Token-UC TO SPI-Current-Token
GC1010         END-IF
               IF Token-Is-Verb
...
GC1010     IF UPPER-CASE(TRIM(PIC-X256,Trailing)) =
GC1010        TRIM(Main-Module-Name)
...
GC0710         END-IF
      ...
      *>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
       510-Control-Record.
           ...
           END-UNSTRING
GC1010     IF UPPER-CASE(TRIM(PIC-X256,Trailing)) =
GC1010        TRIM(Main-Module-Name) *> Main Pgm
               SET In-Main-Module TO TRUE
               IF Source-Line-No > 0
